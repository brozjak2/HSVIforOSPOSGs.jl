...
function ParsedGameDefinition(game_file_path::String)
    return open(game_file_path, "r") do file
        ...
        return ParsedGameDefinition(game_params, states_names, states_partitions,
            ...
            follower_actions, leader_actions, transitions, rewards, init_partition,
            init_belief)
    end
end
...
parse_actions(file::IO, count::Int64) = [parse_ints_plus_one(readline(file)) for i = i:count]
parse_ints_plus_one(line::String) = [parse(Int64, x) + 1 for x in split(line, ' ')]

parse_transitions(file::IO, count::Int64) = [Transition(readline(file)) for i = i:count]
parse_rewards(file::IO, count::Int64) = [Reward(readline(file)) for i = i:count]
